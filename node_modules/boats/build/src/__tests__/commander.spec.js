...
jest.mock('@/commander.helperFunctions', () => (path) => [path]);
describe('cli param parsing', () => {
    it('check the right stuff is returned', () => {
        const commanderReturn = commander_1.default(['node', 'myscript', '--init', '-i', 'bill', '-o', 'ben', '-x']);
    ...
    it('all the short options', () => {
        // shorts
        const commanderReturn = commander_1.default([
            'node',
            'myscript',
            '-i',
            'inpath',
