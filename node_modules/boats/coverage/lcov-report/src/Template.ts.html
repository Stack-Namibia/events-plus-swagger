...
<head>
    <title>Code coverage report for src/Template.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../index.html">All files</a> / <a href="index.html">src</a> Template.ts</h1>
        <div class='clearfix'>
            
            <div class='fl pad1y space-right2'>
                <span class="strong">63.8% </span>
                <span class="quiet">Statements</span>
                <span class='fraction'>104/163</span>
            </div>
        
            
            <div class='fl pad1y space-right2'>
                <span class="strong">48.93% </span>
                <span class="quiet">Branches</span>
                <span class='fraction'>23/47</span>
            </div>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">import { readdirSync, statSync } from 'fs';
...
class Template {
  ...
  mixinObject: any[];
  mixinNumber: number;
  in<span class="fstat-no" title="function not covered" >de</span>ntObject: any[];<span class="branch-0 cbranch-no" title="branch not covered" ><span class="branch-0 cbranch-no" title="branch not covered" ></span></span>
  indent<span class="cstat-no" title="statement not covered" >Number: number;<span class="fstat-no" title="function not covered" ></span></span>
<span class="cstat-no" title="statement not covered" ></span>
   ...
   * @param <span class="cstat-no" title="statement not covered" >inputFile The input directory to start pars</span>ing from
   * @param <span class="cstat-no" title="statement not covered" >output The directory to output/mirror to</span>
   * @param <span class="cstat-no" title="statement not covered" >originalIndent The original indent (currently hard</span> coded to 2)
   * @param <span class="cstat-no" title="statement not covered" >stripValue The strip value for the</span> uniqueOpIp
   * @param <span class="cstat-no" title="statement not covered" >variables The variables </span>for the tpl engine
   * @param <span class="cstat-no" title="statement not covered" >helpFunctionPaths Array of fully qualified local file paths to </span>nunjucks helper functions
   * @param <span class="cstat-no" title="statement not covered" >boatsrc</span>
   */
  // eslint-disable-next-line max-lines-per-function
  directoryParse (<span class="cstat-no" title="statement not covered" ></span>
    inputFil<span class="cstat-no" title="statement not covered" >e: string,</span>
    output: string,
    original<span class="cstat-no" title="statement not covered" >Indent = defaults.DEFAULT_ORIGINAL_INDENTATION,</span>
    stripValue = defaults.DE<span class="fstat-no" title="function not covered" >FA</span>ULT_STRIP_VALUE,
    variables: a<span class="cstat-no" title="statement not covered" >ny[],</span>
    helpFunctionPath<span class="cstat-no" title="statement not covered" >s: string[],</span>
    boatsrc: BoatsRC<span class="cstat-no" title="statement not covered" ></span>
  ): Promise&lt;string&gt; {<span class="cstat-no" title="statement not covered" ></span>
    return new Promi<span class="cstat-no" title="statement not covered" >se((resolve, reject) =&gt; {</span>
      if (!inputFile || <span class="cstat-no" title="statement not covered" >!output) {</span>
...
&nbsp;
      ...
      this.mixinVarN<span class="cstat-no" title="statement not covered" >amePrefix = defaults.DEFAULT_MIXIN_VAR_PREFIX;</span>
      this.helpFunct<span class="cstat-no" title="statement not covered" >ionPaths = helpFun</span>ctionPaths || [];
      this.variables = variables || [];
      this.boatsrc = boatsrc;
      this.inputFile = inputF<span class="fstat-no" title="function not covered" >il</span>e = this.cleanInputString(inputFile);
      this.isAsy<span class="cstat-no" title="statement not covered" >ncApiFile = isAsyncApi(this.inputFi</span>le);
&nbsp;
      // ensure we parse th<span class="fstat-no" title="function not covered" >e </span>input file 1st as this typically contains the inject function
      // this wi<span class="cstat-no" title="statement not covered" >ll also allow us to determine the api type and cor</span>rectly set the stripValue
...
&nbsp;
      le<span class="missing-if-branch" title="if path not taken" >I</span>t returnFileinput: string;
      walker<span class="cstat-no" title="statement not covered" >(upath.dirname(inpu</span>tFile))
        .on('file', (file: string) =&gt; {
          try {
...
<span class="fstat-no" title="function not covered" ></span>          reject(er + ' on entry ' + entry);
<span class="cstat-no" title="statement not covered" >        })</span>
<span class="cstat-no" title="statement not covered" >        .on('end', () =&gt; {</span>
          re<span class="cstat-no" title="statement not covered" >solve(this.stripNjkExtension(returnFileinput));</span>
        });
    });<span class="cstat-no" title="statement not covered" ></span>
  }<span class="cstat-no" title="statement not covered" ></span>
&nbsp;
  setDef<span class="cstat-no" title="statement not covered" >aultStripValue (stripValu</span>e?: string, inputString?: string): string {
    if (stripValue) {
      return stripValue;
    }
    switch (apiTypeFromString(inputString)) {
      ...
      case 'openapi':
        return 'src/paths/';
      case 'asyncapi':
<span class="fstat-no" title="function not covered" ></span>        return 'src/channels/';
    }<span class="cstat-no" title="statement not covered" ></span>
    thro<span class="cstat-no" title="statement not covered" >w new Error('Non supported api type provided. BOATS only supports swagger/openapi/asyncapi');</span>
  }
&nbsp;
  /**
...
<span class="fstat-no" title="function not covered" ></span>      return relativeFilePath.substring(1, relativeFilePath.length);
    }<span class="cstat-no" title="statement not covered" ></span>
    return relativeFil<span class="cstat-no" title="statement not covered" >ePath;</span>
  }<span class="cstat-no" title="statement not covered" ></span>
&nbsp;
  /**
   ...
   * @param inputFile
   * @param currentFile
   *<span class="fstat-no" title="function not covered" > @</span>param outputDirectory
   * @re<span class="cstat-no" title="statement not covered" >turns {*}</span>
   */<span class="cstat-no" title="statement not covered" ></span>
  calcul<span class="cstat-no" title="statement not covered" >ateOutputFile (inputFi</span>le: string, currentFile: string, outputDirectory: string) {
    cons<span class="cstat-no" title="statement not covered" >t inputDir = upath.dirname(inputFile);</span>
    retu<span class="cstat-no" title="statement not covered" >rn this.stripNjkExtensi</span>on(upath.join(process.cwd(), outputDirectory, currentFile.replace(inputDir, '')));
  }<span class="cstat-no" title="statement not covered" ></span>
<span class="cstat-no" title="statement not covered" ></span>
  /**<span class="cstat-no" title="statement not covered" ></span>
...
&nbsp;
  ...
  renderFile (inputString: string, fileLocation: string) {
    ...
    this.nunjucksSetup();
&nbsp;
<span class="fstat-no" title="function not covered" >    co</span>nst renderedYaml = Injector.injectAndRender(<span class="branch-0 cbranch-no" title="branch not covered" >fi</span>leLocation, this.inputFile, this.boatsrc, this.isAsyncApiFile);
<span class="cstat-no" title="statement not covered" ></span>
...
<span class="cstat-no" title="statement not covered" ></span>
   ...
   * @pa<span class="cstat-no" title="statement not covered" >ram originalIndentation The original indentation setting, defaults to 2</span>
   * @returns {Array}<span class="cstat-no" title="statement not covered" ></span>
   */
  setMixinPositions (str: string, originalIndentation = 2): any[] {
    const regexp = RegExp(/(mixin\(["'`]([^"`']*)["'`].*\))/, 'g');
    let matches;
    const matched: any[] = <span class="cstat-no" title="statement not covered" >[];</span>
    while ((<span class="cstat-no" title="statement not covered" >matches = reg<span class="cstat-no" title="statement not covered" >ex</span>p.exec(str)) !== null) {</span>
      const mixi<span class="cstat-no" title="statement not covered" >nObj = {</span>
        index: regexp.lastIndex,
        matc<span class="cstat-no" title="statement not covered" >h: matches[0],</span>
        mixinPath: matches[2],
<span class="cstat-no" title="statement not covered" >        mixinLinePadding</span>: '',
      };
...
<span class="missing-if-branch" title="if path not taken" >I</span>
    while ((<span class="cstat-no" title="statement not covered" >matches = regexp.exec(p<span class="fstat-no" title="function not covered" >re</span>paredString)) !== null) {</span>
      const indentObject = {<span class="cstat-no" title="statement not covered" ></span>
        index: r<span class="cstat-no" title="statement not covered" >egexp.lastIndex,</span>
        match: matches[0],
        linePadding: '',
      };
...
&nbsp;
    ...
    env.addG<span class="missing-if-branch" title="else path not taken" >E</span>lobal('autoTag', autoTag);
...
&nbsp;</pre></td></tr></table></pre>
            ...
            <div class='footer quiet pad2 space-top1 center small'>
                ...
                at Wed Jun 08 2022 15:58:54 GMT+0100 (Western European Summer Time)
            </div>
